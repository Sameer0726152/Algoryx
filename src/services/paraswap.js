...
// No API key needed - Paraswap is completely free and open
...
export async function getParaswapQuote(fromToken, toToken, amount) {
  ...
  const url = `${BASE_URL}/prices?srcToken=${srcToken}&destToken=${destToken}&amount=${amountWei}&srcDecimals=${srcDecimals}&destDecimals=${destDecimals}&side=SELL&network=1`

  const response = await fetch(url)

  if (!response.ok) {
    throw new Error(`Paraswap failed: ${response.status}`)
  }
